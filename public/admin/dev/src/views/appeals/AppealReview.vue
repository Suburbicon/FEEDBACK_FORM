    ...
    methods: {
        ...
        onSubmitModal() {
            this.$validator.validateAll().then((result) => {
                ...
                if (this.action === 'add') {
                    this.url = `/appeal/${this.action}?name=${this.items.name}`;
                } else if (this.action === 'edit') {
                    this.url = `/appeal/${this.action}?id=${this.items.id}&name=${this.items.name}`;
                }
                ...
                axios.post(this.url)
                    .then(response => {
                        ...
                        this.$store.dispatch('appeal/getAppealsDataInStorage')
                            .catch(error => {
                                console.log(error)
                            })
                    })
